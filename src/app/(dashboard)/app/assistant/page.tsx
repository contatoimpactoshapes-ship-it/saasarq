...
function scoreStyle(score: number) {
  if (score >= 90) return { num: "text-emerald-600", bar: "bg-emerald-500", badge: "bg-emerald-50 text-emerald-700 border border-emerald-200",  label: "Excelente" };
  if (score >= 80) return { num: "text-indigo-600",  bar: "bg-indigo-500",  badge: "bg-indigo-50 text-indigo-700 border border-indigo-200",    label: "Forte" };
  if (score >= 70) return { num: "text-amber-500",   bar: "bg-amber-500",   badge: "bg-amber-50 text-amber-700 border border-amber-200",       label: "Bom" };
  return           { num: "text-rose-500",    bar: "bg-rose-500",    badge: "bg-rose-50 text-rose-700 border border-rose-200",           label: "A Melhorar" };
}
  ...
  return (
    <div className="bg-white border border-zinc-200/80 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400">Score de Qualidade Arquitetônica</span>
...
function StyleCard({ prompt }: { prompt: string }) {
  ...
  return (
    <AnalysisCard icon={<Palette className="w-3 h-3" />} title="Estilo">
...
function MaterialsCard({ prompt }: { prompt: string }) {
  ...
  return (
    <AnalysisCard icon={<Layers className="w-3 h-3" />} title="Materiais">
      {materials.length > 0 ? (
        ...
        <p className="text-xs text-zinc-400">Nenhum material identificado.</p>
      )}
    </AnalysisCard>
  );
}
...
function LightingCard({ prompt }: { prompt: string }) {
  ...
  return (
    <AnalysisCard icon={<Sun className="w-3 h-3" />} title="Iluminação">
...
function CameraCard({ prompt }: { prompt: string }) {
  ...
  return (
    <AnalysisCard icon={<Aperture className="w-3 h-3" />} title="Câmera">
      <div className="space-y-1.5">
        {[
          { k: "Focal", v: focalLength },
          { k: "Altura", v: height },
          { k: "Lente",  v: type },
        ].map(({ k, v }) => (
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between py-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-400">Proporção</span>
          </div>
    ...
    >
      ...
      <div className="bg-white border border-zinc-200/80 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-100 bg-zinc-50/80">
          <span className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400">Prompt Gerado</span>
          <button
            onClick={copyPrompt}
            className="flex items-center gap-1.5 text-[10px] font-medium text-zinc-500 hover:text-zinc-900 transition-colors"
          >
            {copied
              ? <><Check className="w-3 h-3 text-emerald-500" /><span className="text-emerald-600">Copiado</span></>
      ...
      {result.suggestions.length > 0 && (
        <Section label="Sugestões de Refinamento">
...
export default function AssistantPage() {
  ...
  async function handleAnalyze(text = input) {
    ...
    if ((!content && !imageFile && !details) || loading) return;

    // Combine input text with project details so Claude receives full context
    let fullMessage = content;
    if (details) {
      const detailsBlock = `Detalhes do projeto fornecidos pelo arquiteto:\n${details}`;
    ...
    try {
      ...
      if (fullMessage) form.append("message", fullMessage);
      if (imageFile)   form.append("image",   imageFile);
  ...
  const inputBar = (
    <div className="flex items-end gap-2 p-1.5 rounded-xl border border-zinc-200 bg-white focus-within:border-zinc-400 transition-colors">
      ...
      <textarea
        ...
        placeholder={pendingImage ? "Instruções adicionais ao Architect (opcional)…" : "Descreva o espaço ou envie uma referência arquitetônica…"}
        rows={1}
        disabled={loading}
          ...
          >
            ...
            <aside className="w-64 shrink-0 border-r border-zinc-200 flex flex-col bg-white">
              ...
              <div className="px-3 py-2 border-b border-zinc-100 flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400">Histórico</span>
              </div>
              ...
              <div className="w-full max-w-lg space-y-5">
                <div className="text-center space-y-1.5">
                  ...
                  <p className="text-sm text-zinc-500 max-w-sm mx-auto leading-relaxed">
                    A IA analisa a referência e gera um prompt fotográfico hiper-realista.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <p className="text-[11px] text-zinc-400 mt-0.5 leading-relaxed">
                      Ajude a IA a entender melhor seu projeto. Informe materiais, acabamentos,
                      mobiliário, iluminação, estilo arquitetônico ou qualquer detalhe importante
                      para a renderização.
                    </p>
                  </div>
                  <textarea
                    ...
                    placeholder={
                      "Exemplos:\n" +
                      "• Piso em porcelanato travertino acetinado 120x120\n" +
                      "• Bancada em quartzito Taj Mahal\n" +
                      "• Marcenaria em carvalho natural\n" +
                      "• Metais em preto fosco\n" +
                      "• Sofá verde oliva\n" +
                      "• Preservar paginação da pedra\n" +
                      "• Manter exatamente o layout da referência\n" +
                      "• Iluminação indireta quente 3000K\n" +
                      "• Estilo contemporâneo brasileiro"
                    }
                    rows={7}
                    className="w-full resize-none text-xs text-zinc-800 placeholder:text-zinc-300 bg-white border border-zinc-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-zinc-400 transition-colors leading-relaxed disabled:opacity-50"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-center text-[10px] text-zinc-400">
                    Enter para analisar · Shift+Enter para nova linha · ícone de upload para imagem
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <aside className="w-64 shrink-0 border-r border-zinc-200 flex flex-col bg-white">
              ...
              <div className="px-3 pt-3 pb-2">
                {studioImage ? (
                  ...
                  <div className="rounded-xl bg-zinc-50 border border-zinc-200 border-dashed flex items-center justify-center aspect-[4/3]">
                    <p className="text-xs text-zinc-400">Análise por texto</p>
                  </div>
                )}
              ...
              <div className="p-3 border-t border-zinc-100 space-y-1.5">
                {inputBar}
                <p className="text-[10px] text-zinc-400 text-center leading-tight">Enter para reanalisar</p>
              </div>
            </aside>
